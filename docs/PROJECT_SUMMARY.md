...
- [x] 2026-06-21 — Payment amount tracking: tambah kolom payment_amount ke orders table untuk webhook verification, migration file dibuat — files: backend/database/migrations/20260621-084842-add-payment-amount-column.sql, backend/src/models/order.model.ts
- [ ] 2026-06-21 — Migration pending: migration payment_amount belum dijalankan karena koneksi database timeout (database remote belum accessible) — status: blocked on database connection
