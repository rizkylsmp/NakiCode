...
router.get('/', requireAdmin, async (req: Request, res: Response) => {
  try {
    ...
    const [recentOrdersResult] = await pool.query(`
      SELECT id, customer_name AS customerName, design_title AS templateTitle,
        budget_range AS budgetRange, status, created_at AS createdAt
      FROM orders
      WHERE deleted_at IS NULL
    ...
    const [weeklyRevenueResult] = await pool.query(`
      SELECT
        YEARWEEK(paid_at, 1) as yearWeek,
        DATE_FORMAT(paid_at, '%Y-%W') as week,
        SUM(payment_amount) as revenue,
        COUNT(*) as orders
      FROM orders
      WHERE deleted_at IS NULL
        AND payment_status = 'paid'
        AND paid_at >= DATE_SUB(NOW(), INTERVAL 8 WEEK)
