...
type CouponForm = Omit<Coupon, "id" | "createdAt" | "redemptionCount" | "expiresAt"> & {
  expiresAt: string;
};
      ...
      <div className="overflow-hidden rounded-xl border border-naki-steel bg-white">
        <div className="overflow-x-auto">
          <table className="w-full min-w-190 text-left">
            <thead className="bg-naki-frost text-xs uppercase text-naki-smoke"><tr><th className="p-4">Kode</th><th className="p-4">Diskon</th><th className="p-4">Berlaku sampai</th><th className="p-4">Dipakai</th><th className="p-4">Status</th><th className="p-4 text-right">Aksi</th></tr></thead>
            <tbody className="divide-y divide-naki-steel">
              {coupons.map((coupon) => {
                ...
                return <tr key={coupon.id} className="text-sm">
                  ...
                  <td className="p-4 text-naki-smoke">{coupon.expiresAt ? new Date(coupon.expiresAt).toLocaleString("id-ID", { dateStyle: "medium", timeStyle: "short" }) : "Tanpa batas"}</td>
                  <td className="p-4 text-naki-primary">{coupon.redemptionCount} order</td>
                  <td className="p-4"><button className={`rounded-full px-3 py-1 text-xs font-semibold ${coupon.active && !expired ? "bg-green-50 text-green-700" : "bg-naki-frost text-naki-smoke"}`} onClick={() => void toggleCoupon(coupon)} type="button">{expired ? "Kedaluwarsa" : coupon.active ? "Aktif" : "Nonaktif"}</button></td>
