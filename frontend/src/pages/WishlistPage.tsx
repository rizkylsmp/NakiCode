      ...
      <section className="w-full px-5 py-10 md:px-8 xl:px-12 2xl:px-16">
        <div className="flex flex-col justify-between gap-3 sm:flex-row sm:items-end">
          <div>
            ...
            <h1 className="mt-2 text-4xl font-black">Template tersimpan</h1>
            <p className="mt-2 text-naki-smoke">
              Simpan template incaran sebelum checkout atau konsultasi.
            </p>
          </div>
          <Link
            className="inline-flex h-11 w-fit items-center justify-center gap-2 rounded-lg bg-naki-secondary px-4 text-sm font-black text-naki-frost transition hover:bg-naki-primary"
            to="/#template"
          >
            Cari template
            <ArrowRight size={16} />
        ...
        {favoriteTemplates.length === 0 ? (
          ...
          <div className="mt-8 grid gap-4">
            {favoriteTemplates.map((template) => (
              <article
                key={template.id}
                className="grid gap-4 rounded-xl border border-naki-steel bg-naki-frost p-4 shadow-naki-card md:grid-cols-[180px_1fr_auto] md:items-center"
              >
                {template.preview[0]?.image ? (
                  <ResponsiveImage
                    className="h-36 w-full rounded-lg object-cover md:h-28"
                    src={template.preview[0].image}
                    sizes="(min-width: 768px) 180px, 100vw"
                    alt={template.preview[0].caption || template.title}
                  />
                ) : (
                  <div className="grid h-36 place-items-center rounded-lg bg-naki-steel md:h-28">
                ...
                <div className="min-w-0">
                  ...
                  <h2 className="mt-1 text-2xl font-black">{template.title}</h2>
                  <p className="mt-2 line-clamp-2 text-sm font-semibold leading-6 text-naki-smoke">
                    {template.description}
                  </p>
                </div>
                <div className="flex gap-2 md:flex-col">
                  <Link
                    className="inline-flex h-10 items-center justify-center gap-2 rounded-lg bg-naki-primary px-4 text-sm font-black text-naki-frost transition hover:bg-naki-secondary"
                  ...
                  <button
                    className="inline-flex h-10 items-center justify-center gap-2 rounded-lg border border-naki-steel px-4 text-sm font-black text-naki-secondary transition hover:border-naki-smoke disabled:text-naki-smoke"
